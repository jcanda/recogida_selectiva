...
class recogida_diario extends fs_model {
    ...
    public function nombre_ayunta($ayunt_id = '') {
        if ($ayunt_id != '')
            $sql = "SELECT entidad_nombre FROM `recogida_entidad` WHERE entidad_id = " . $this->var2str($ayunt_id) . ";";
        else
    ...
    public function search_ayunta($ayunta = '') {
        $entidad_model = new recogida_entidad();
        return $entidad_model->search($ayunta, 1);
    }

    public function search_empresa($empre = '') {
        $entidad_model = new recogida_entidad();
        return $entidad_model->search($empre, 2);
    }
    ...
    public function nombre_material($material_id = '') {
        $materiales = $this->materiales();
        if ($material_id != '')
            return $materiales[$material_id];
        else
            return $materiales[$this->material_id];
    }

    public function sumabytipo($buscar = '', $desde = '', $hasta = '', $material = '', $tipo = '') {
        $sql = "SELECT SUM(entrada) as total "
                . "FROM `recogida_diario` INNER JOIN `recogida_entidad` ON recogida_entidad.entidad_id = recogida_diario.entidad_id "
                . "WHERE upper(entidad_nombre) LIKE upper('%" . $buscar . "%')";
        ...
        if ($material != '') {
            $sql .= " AND `material_id` = " . $this->var2str($material);
        }
    ...
    public function search($buscar = '', $desde = '', $hasta = '', $material = '', $ayunta = '', $orden = "fecha") {
        $entidadlist = array();

        $sql = "SELECT recogida_diario.*, recogida_entidad.entidad_nombre
         FROM `recogida_diario` INNER JOIN recogida_entidad ON recogida_entidad.entidad_id = recogida_diario.entidad_id
         WHERE recogida_id > 0";
